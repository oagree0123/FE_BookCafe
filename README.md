...
## 🐛 트러블 슈팅
<br/>
1. axios를 사용할 때, header에 token이 담겨서 넘겨지지 않는 문제
   axios.post(url,header) 형태로 코드를 작성하지 axios의 post 요청 형태인 axios.post(url, data, header)가 지켜지지 않아, header가 아닌
   data로 넘어가는 것을 확인해, data 위치에 {} 빈 객체를 선언하여 해결했습니다.
   
2. 토큰을 전역 변수로 가져오려고 하니 안받아지는 문제
   api헤더에 넘기기 위해서 세션에 저장한 토큰을 꺼내 사용하려고 하는데
   전역 변수로 작성하니 값을 읽지 못해서, 블록 스코프마다 토큰을 가져오도록 설정했습니다.
